<?php
/* src/View/Helper/AjaxHelper.php */
 ...
 * Updated to Cake 3.0b by Michael Gaiser 2014
 *
...
class AjaxHelper extends AppHelper {
	 ...
	 * @public array
	 */
	public $helpers = ['Html', 'Url', 'Form', 'Javascript'];
 ...
 */
	public function form($params = null, $type = 'post', $options = array()) {
		...
		$defaults = ['model' => $model, 'with' => "$('#{$htmlOptions['id']}').serialize()", 'type' => $type];
...
/**
 * Observe field and call ajax on change, Then populate a select field.
 *
 * Observes the field with the DOM ID specified by <i>field</i> and makes
 * an Ajax when its contents have changed.
 *
 ...
 * @return string ajax script
 */
	public function autoPopulate($observeField, $targetFields, $options = array()) {
		
		//Setup the observe field
		$observeFieldName = explode(".", $observeField);
		$observeModelName = $observeFieldName[0];
		$observeFieldName = $observeFieldName[1];	
		$observeField = $observeModelName.Inflector::camelize($observeFieldName);
		
		//setup the target fields
		$targets = [];
		foreach($targetFields as $targetField){
			$targetFieldName = explode(".", $targetField);
			array_push($targets, $targetFieldName[0].Inflector::camelize($targetFieldName[1]));
		}
		//debug($targets);
		
		if (!isset($options['with'])) {
		    $options['with'] = "$('#$observeField').serialize()";
		}

		if (isset($options['conditions'])) {
			$conditions = base64_encode(serialize($options['conditions']));
			$options['url']['conditions'] = $conditions;
			unset($options['conditions']);
		}
		
		if (isset($options['find'])) {			
			$options['url']['find'] = $options['find'];
			unset($options['find']);
		}
		...
		$options['url']['model'] = $observeModelName;
		$options['url']['field'] = $observeFieldName;
		...
		if($empty){
			$cmd .= "options += '<option value=\"\"></option>';";
		}
		$cmd .= "
					if(v instanceof Object){
						options += '<optgroup label=\"' + k + '\">';
						$.each(v, function(y, z) {
							options += '<option value=\"' + y + '\">' + z + '</option>';
						});
						options += '</optgroup>';
					}else{
		...
		$callback = $this->remoteFunction($options);
		return $this->Javascript->event("'#{$observeField}'", "change", $callback);
	}	
...
//'nameFormat'=>array('format'=>'%s (%s)', 'fields'=>array('Domain.name', 'ParentDomain.name'))
//'conditions'=>array('Character.chronicle_id'=>(int)$this->Session->read('game.chronicle_id')
	...
	public function autoComplete($field, $options = array()) {
		...
		$results = ['modelName'=>null, 'fieldName'=>null, 'conditions'=>null, 'nameFormat'=>null];

		switch(count($fieldName)){
			default:
			case 2:
				$results['modelName'] = $fieldName[0];
				$results['fieldName'] = $fieldName[1];
				break;

			case 3:
				$results['modelName']= $fieldName[0];
				$results['fieldName'] = $fieldName[2];
				break;
		}

		if(isset($options['fieldName'])){
			$override = explode(".", $options['fieldName']);
			switch(count($override)){
				default:
				case 2:
					$field = $override[0].'.'.$override[1];
					break;
					
				case 3:
					$field = $override[0].'.'.$override[1].'.'.$override[2];
					break;
			}
		}
		...
		if(isset($options['fieldIdName'])){
			$override = explode(".", $options['fieldIdName']);
			switch(count($override)){
				default:
				case 2:
					$idField = $override[0].'.'.$override[1];
					break;
				case 3:
					$idField = $override[0].'.'.$override[1].'.'.$override[2];
					break;
			}
		}
		...
		if (!isset($options['id'])) {
			$options['id'] = strtolower(str_replace(['.', '_'], "-", $field));
		}
		if (!isset($options['idid'])) {
			$options['idid'] = strtolower(str_replace(['.', '_'], "-", $idField));
		}

		if (isset($options['passId'])) {
			if($options['passId']){
				$passId = true;
			}
			unset($options['passId']);
		}

		if (isset($options['conditions'])) {

			$results['conditions'] = $options['conditions'];
			//$conditions = sprintf('/conditions:%s', base64_encode(serialize($options['conditions'])));
			unset($options['conditions']);
		}
		
		if (isset($options['nameFormat'])) {
			$results['nameFormat'] = $options['nameFormat'];
			//$nameFormat = sprintf('/name_format:%s', base64_encode(serialize($options['nameFormat'])));
			unset($options['nameFormat']);
		}
		...
		$encodedResults = base64_encode(serialize($results));
		$url = ['controller'=>$results['modelName'], 'action'=>'autoComplete', 'data'=>$encodedResults];
		$text = $this->Form->input($field, $htmlOptions);
		$script = "{$var} $('#{$htmlOptions['id']}').autocomplete('";
		$script .= $this->Url->build($url) . "', {$options});";
		if($passId){
			$text .= "\n".$this->Form->input($idField, ['type'=>'hidden', 'id'=>$htmlOptions['idid']]);
			$script .= "\n ".sprintf('$("#%s").autocomplete().result(function(evt,data,formatted){$("#%s").val(data[1]); $("#%s").change(); });', $htmlOptions['id'], ($htmlOptions['idid']), ($htmlOptions['idid']));
		}
		return  "{$text}\n" . $this->Html->scriptBlock($script, ['block'=>false]);
	}
 ...
 */
	public function drop($id, $options = array()) {
		...
		$callbacks = ['activate', 'deactivate', 'over', 'out', 'drop'];
 ...
 */
	public function slider($id, $options = array()) {
		...
		$options = $this->_optionsToString($options, ['orientation', 'range']);
		$callbacks = ['change', 'slide', 'stop', 'start'];
 ...
 */
	public function resizable($id, $options = array()) {
		if (isset($options['var'])) {
			$var = 'var ' . $options['var'] . ' = ';
			unset($options['var']);
		} else {
		...
		$options = $this->_optionsToString($options, ['animateEasing', 'cancel', 'helper', 'containment']);
 ...
 */
	public function selectable($id, $options = array()) {
		if (isset($options['var'])) {
			$var = 'var ' . $options['var'] . ' = ';
			unset($options['var']);
		} else {
		...
		$options = $this->_optionsToString($options, ['cancel', 'filter', 'tolerance']);
		$callbacks = ['selected', 'selecting', 'start', 'stop', 'unselected', 'unselecting'];
 ...
 */
	public function accordion($id, $options = array()) {
		if (isset($options['var'])) {
			$var = 'var ' . $options['var'] . ' = ';
			unset($options['var']);
		} else {
		...
		$options = $this->_optionsToString($options, ['event']);
		$callbacks = ['change', 'changestart', 'navigationFilter'];
 ...
 */
	public function button($id, $options = array()) {
		if (isset($options['var'])) {
			$var = 'var ' . $options['var'] . ' = ';
			unset($options['var']);
		} else {
		...
		$options = $this->_optionsToString($options, ['label']);
 ...
 * @param array $options Array of options to control the buttonset
     * @link          http://docs.jquery.com/UI/Button
 */
	public function buttonset($id, $options = array()) {
		if (isset($options['var'])) {
			$var = 'var ' . $options['var'] . ' = ';
			unset($options['var']);
		} else {
 ...
 */
	public function datepicker($id, $options = array()) {
		if (isset($options['var'])) {
			$var = 'var ' . $options['var'] . ' = ';
			unset($options['var']);
		} else {
		...
		$callbacks = ['beforeShow', 'beforeShowDay', 'onChangeMonthYear', 'onClose', 'onSelect'];
 ...
 */
	public function dialog($id, $options = array()) {
		...
		$callbacks = ['beforeClose', 'open', 'focus', 'dragStart', 'drag', 'dragStop', 'resizeStart', 
		'resize', 'resizeStop', 'close'];
 ...
 */
	public function progressbar($id, $options = array()) {
		if (isset($options['var'])) {
			$var = 'var ' . $options['var'] . ' = ';
			unset($options['var']);
		} else {
 ...
 */
	public function tabs($id, $options = array()) {
		if (isset($options['var'])) {
			$var = 'var ' . $options['var'] . ' = ';
			unset($options['var']);
		} else {
		...
		$callbacks = ['select', 'load', 'show', 'add', 'remove', 'enable', 'disable'];
 ...
 */
	public function editor($id, $url, $options = array()) {
		...
		$callbacks = ['callback', 'onsubmit', 'onreset', 'onerror'];
 ...
 */
	public function sortable($id, $options = array()) {
		...
		$callbacks = ['start', 'sort', 'change', 'beforeStop', 'stop', 'update', 
	...
	public function __optionsForAjax($options) {
		...
		foreach ($options as $key => $value) {
			switch ($key) {
				...
				break;
				default:
				if (!in_array($key, $this->callbacks) && !in_array($key, ['before', 'after', 'confirm', 'condition', 'interactive', 'update']))
				    $jsOptions[$key] = $value;
				break;
			}
		}
